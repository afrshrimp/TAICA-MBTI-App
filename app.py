...
# --- 1. 頁面配置與櫻花馬卡龍系視覺美化 ---
st.set_page_config(page_title="TAICA MBTI V15.3 🌸 (Clean Frontend Edition)", page_icon="👑", layout="wide")
...
# 階段 C: 最終 AI 深度報告 (搭載 Prompt 邏輯鎖與前台日誌分離)
else:
    if 'final_report' not in st.session_state:
        with st.spinner("✨ 測驗完成！大師正在雲端利用 Chain-of-Thought 思考鏈精算數據..."):
            ...
            步驟 2：逐題分析使用者的回答，判斷其傾向（包含模糊運算評估）。
            步驟 3：加總分數，嚴格依照上述【絕對計分準則】得出 4 個字母。
            步驟 4：關閉 </thought_process> 標籤。
            步驟 5：輸出標準的 JSON 區塊。
            步驟 6：撰寫診斷報告。報告內容必須完全符合你算出的 MBTI 字母！

            【完美輸出格式範本】
            <thought_process>
            - 題1分析：喜歡聚會，強烈 E 傾向。模糊程度：80
            - ...逐題分析完畢。
            ...
            * (根據真實算出的 MBTI 撰寫貼心建議...)
            ...
            mbti_type = "未定義"
            clean_report = raw_res

            try:
                # 1. 容錯 JSON 解析 (擷取分數與類型)
                ...
                # 在伺服器後台 (VS Code 終端機或 Streamlit Cloud後台) 印出詳細計算過程
                print(f"\\n=== [後台監控] {st.session_state.nickname} 的 MBTI 運算日誌 ===")
                print(backend_log)
                print("=====================================================\\n")

                # 清理出要給使用者的乾淨報告
                # 直接鎖定 "### 🌸 TAICA" 標題，只保留標題以下的內容
                if "### 🌸 TAICA" in raw_res:
                    clean_report = "### 🌸 TAICA" + raw_res.split("### 🌸 TAICA")[1]
                else:
                    # 如果 AI 沒生成標準標題，就用暴力法把 thought_process 徹底刪除
                    clean_report = re.sub(r'<thought_process>.*?</thought_process>', '', raw_res, flags=re.DOTALL | re.IGNORECASE).strip()
                    # 順便把 json 區塊也清掉
                    clean_report = re.sub(r'```json.*?```', '', clean_report, flags=re.DOTALL | re.IGNORECASE).strip()

            except Exception as e:
                print(f"解析發生錯誤: {e}")
                clean_report = "系統正在進行深度腦力激盪，請稍後再試！"
